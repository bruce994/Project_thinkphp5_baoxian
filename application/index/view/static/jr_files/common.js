...
//渲染分页
function renderSCPager(first,prev,next,last)
{
	var TotalPage=0;
	...
	for(var i=beginI;i<=TotalPage;i++)
	{
		if(i==CurrentPage)
		{
			SCpager=SCpager+"<a class=\"pgcurrent\">"+i+"</a>";
		}
		else
		{
			SCpager=SCpager+"<a href=\""+PageName+i+"\">"+i+"</a>";
		}
		loopi++;
		if(loopi==9) break;
	}
	...
	SCpager=SCpager+endpager;
	SCpager=SCpager+"</div>";
	get("SC_pager").innerHTML=SCpager;
}

//豪华版渲染分页
function renderSCPager2(first,prev,next,last)
{
	var TotalPage=0;
	...
	SCpager=SCpager+"共有："+TotalRecord+"条&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	var startpager="<a class=pgNext href=\""+PageName+"1\">"+first+"</a>\n<a class=pgNext href=\""+PageName+(CurrentPage-1)+"\">"+prev+"</a>\n";
	if(CurrentPage==1)
	{
		startpager="<a class=\"pgnext pgempty\">"+first+"</li>\n<a class=\"pgnext pgempty\">"+prev+"</a>\n";
	...
	SCpager=SCpager+"页次:"+CurrentPage+"/"+TotalPage+"页";
	
	//SCpager=SCpager+"<select onchange='location.href=\""+PageName+"this.value\"'>";
	SCpager=SCpager+"<select onchange='location.href=\""+PageName+"\"+this.value'>";
	for(var i=beginI;i<=TotalPage;i++)
	{
         if(i==CurrentPage)
		 {
     	 SCpager=SCpager+"<option value='"+i+"' selected=\"selected\">"+i+"</option>";
     	 }
     	 else
     	 {
     	   SCpager=SCpager+"<option value='"+i+"'>"+i+"</option>";
     	 }
	}
	
	SCpager=SCpager+"</select>";
	SCpager=SCpager+"</div>";
	get("SC_pager").innerHTML=SCpager;
}
//英文豪华版渲染分页
function renderSCPager3(first,prev,next,last)
{
	var TotalPage=0;
	...
	SCpager=SCpager+"Total："+TotalRecord+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	var startpager="<a class=pgNext href=\""+PageName+"1\">"+first+"</a>\n<a class=pgNext href=\""+PageName+(CurrentPage-1)+"\">"+prev+"</a>\n";
	if(CurrentPage==1)
	{
		startpager="<a class=\"pgnext pgempty\">"+first+"</li>\n<a class=\"pgnext pgempty\">"+prev+"</a>\n";
	...
	SCpager=SCpager+"Page:"+CurrentPage+"/"+TotalPage;
...
//渲染分页 静态
function renderHtmlSCPager(first,prev,next,last)
{
	var TotalPage=1;
	if(ChkObjectIsExists("SCPage_PageInfo"))
	{
		var TotalPage=get("SCPage_PageInfo").innerHTML.split("|")[0];
	}
	if(!isNaN(TotalPage))
	{
	...
	for(var i=beginI;i<=TotalPage;i++)
	{
		...
		{
			SCpager=SCpager+"<a href=\""+PageName+"page="+i+".html\">"+i+"</a>";
		}
		loopi++;
		if(loopi==9) break;
	}
